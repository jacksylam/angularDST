...
export class WorkerReadyMethodsService {
  ...
  constructor() { }
    ...
    private getInternalIndices(args: {geojsonObjects: any, options: {background?: boolean, breakdown?: boolean}}): {internal: number [], background?: number, breakdown?: {internal: number[], background?: number}[]} {
      ...
      let getPolygonInternalIndices = (coordinates: number[][][], xs: number[], ys: number[], lcVals: number[], gridWidthCells: number, gridHeightCells: number, mechanisms: {internal: (val: number) => any, background?: (val: number) => any, breakdownInternal?: (val: number) => any, breakdownBackground?: (val: number) => any}): void => {
